...
void render_default_bootlogo()
{
	...
	bitmap = (u8*)sd_file_read("AtomNX/res/launchsplash.bmp", NULL);// Bootlogo Custom
...
void check_power_off_from_hos()
{
	...
	if (hosWakeup & MAX77620_IRQ_TOP_RTC_MASK)
	{
		...
		{
				gfx_clear_grey(0x1B);  //Custom Bootlogo
...
void ini_list_launcher()
{
	...
	if (!ini_parse(&ini_list_sections, "AtomNX/ini", true))
	{
...
void launch_firmware()
{
	...
	if (!ini_parse(&ini_sections, "AtomNX/sys_ipl.ini", false))
	{
		EPRINTF("Could not open 'AtomNX/sys_ipl.ini'!");
		goto parse_failed;
	}
...
void nyx_load_run()
{
	...
	u8 *nyx = sd_file_read("AtomNX/sys/AtomNX-gui.bin", NULL);
	if (!nyx)
		return;
	...
	if (nyx_ver < expected_nyx_ver)
	{
		...
		gfx_con_setpos(0, 0);
		WPRINTF("Old AtomNX GUI found! Seperation of reality will occur!\n");
		WPRINTF("\nUpdate AtomNX folder!\n\n");
...
// Autolaunch Payload if autolaunch.bin in atom folder else load GUI or hold Vol autolaunch abort  // Credits to Storm 
/*static void _auto_launch_firmware()
 {
	if (!f_stat("AtomNX/autopayload.bin", NULL))
	{
		goto out;
	}
	...
	else
	{
		...
		if (!f_stat("AtomNX/autopayload.bin", true)
		{
		...
		payload_path = "AtomNX/autopayload.bin";
...
void auto_launch_update()
{
	...
	else if (sd_mount())
	{
		...
		if (!f_stat("AtomNX/atom.bin", NULL))
			launch_payload("AtomNX/atom.bin", true, false);
		else
		{
			u8* buf = calloc(0x200, 1);
			is_ipl_updated(buf, "AtomNX/atom.bin", true);
			free(buf);
		}
	}
}
...
static void _auto_launch_firmware()
{
	...
	if (f_stat("AtomNX/sys_ipl.ini", NULL))
		create_config_entry();

	// Parse Atom main configuration.
	if (!ini_parse(&ini_sections, "AtomNX/sys_ipl.ini", false))
		goto out; // Can't load sys_ipl.ini.
...
skip_list:
	...
	if (!(b_cfg.boot_cfg & BOOT_CFG_FROM_LAUNCH) && h_cfg.bootwait)
	{
		...
		if (!bitmap) // Custom entry bootlogo not found, trying default custom one.
			bitmap = (u8*)sd_file_read("AtomNX/launchsplash.bmp", &fsize);
...
void ipl_main()
{
	...
	if (!ianos_loader("AtomNX/sys/lp0.bso", DRAM_LIB, sdram_params))
...
skip_lp0_minerva_config:
	...
	// AtomNX Custom Payload Bootlogo, Seperate from AtomNX splas
	sd_mount();
	u8* BOOTLOGO = NULL;
	...
	bitmap = (u8*)sd_file_read("AtomNX/splash.bmp", NULL);
