...
#define ARGON_ORANGE        LV_COLOR_HEX(0x0045ff) //Currently Blue
...
static void basic_init(void)// Mod
{
	...
	theme.bg = &bg;//Always in style. remove xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
	//theme.scr = &scr;
	theme.panel = &panel;
}
...
static void list_init(void)
{
	...
	pr.body.empty = 0;
	// pr.body.radius = 0;
    pr.text.font = &interui_20;// Text size selected --- Font size Style Focus on button

	lv_style_copy(&tgl_rel, &pr);
	tgl_rel.body.main_color = COLOR_HOS_BG_LIGHT;
	tgl_rel.body.grad_color = tgl_rel.body.main_color;
	// tgl_rel.text.color = lv_color_hsv_to_rgb(_hue, 5, 95);
	tgl_rel.text.color = COLOR_HOS_TEAL_LIGHTER;// tgl_rel.text.color = COLOR_HOS_TEAL_LIGHTER;Color list selected  LV_COLOR_WHITE
	tgl_rel.text.font = &interui_20;// Text size selected
	...
	tgl_pr.body.grad_color = tgl_pr.body.main_color;
	tgl_pr.body.border.width = 0;
	// tgl_pr.text.font = &interui_20;// Text size selected --- already 20
...
static void tabview_init(void)
{
	...
	indic.body.main_color = ARGON_ORANGE;// Bar active tab
	indic.body.grad_color = indic.body.main_color;
	indic.body.radius = 0;
	indic.body.border.width = 0;
	indic.body.padding.inner = LV_DPI / 20;// Active bar thickness
	...
	btn_bg.text.color = ARGON_ORANGE;// Tabview text color inactive tab
	//btn_bg.text.color = LV_COLOR_WHITE;// Tabview text color inactive tab
	...
	tgl_rel.text.color = ARGON_ORANGE;// Tabview text color active tab
	//tgl_rel.text.color = LV_COLOR_WHITE;// Tabview text color active tab 
...
static void win_init(void)
{
	...
	lv_style_copy(&rel, &def);//New hekate	    lv_style_copy(&rel, theme.btn.rel);
	rel.body.radius = 0;
 ...
 */
lv_theme_t * lv_theme_cwad_init(uint16_t hue, lv_font_t * font)
{
 ...
 */
lv_theme_t * lv_theme_get_cwad(void)
{
	return &theme;
}
