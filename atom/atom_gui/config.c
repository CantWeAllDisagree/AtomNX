...
int create_config_entry()
{
	...
	{
		u8 res = f_open(&fp, "AtomNX/sys_ipl.ini", FA_READ);
		if (res == FR_NO_FILE || res == FR_NO_PATH)
		{
			f_mkdir("AtomNX");
			f_mkdir("AtomNX/ini");
			f_mkdir("AtomNX/payloads");
			f_mkdir("AtomNX/sys");
		}
		else
		{
	...
	if (f_open(&fp, "AtomNX/sys_ipl.ini", FA_WRITE | FA_CREATE_ALWAYS) != FR_OK)
		return 1;
...
int create_nyx_config_entry(bool force_unmount)
{
	...
	if (f_open(&fp, "AtomNX/gui.ini", FA_WRITE | FA_CREATE_ALWAYS) != FR_OK)
		return 1;
