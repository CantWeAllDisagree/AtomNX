...
skip_main_cfg_parse:
	if (!ini_parse(&ini_nyx_sections, "AtomNX/atom_gui.ini", false))
		return;
...
static void _show_errors()
{
	...
	if (*excp_enabled == EXCP_MAGIC)
	{
		...
		WPRINTFARGS("AtomNX experienced a exception!!! (LR %08X):\n", *excp_lr);
		switch (*excp_type)
		{
...
void nyx_init_load_res()
{
	...
	// Reset new info if magic not correct.															// Figure out what magic does / may cause issues
	...
	load_saved_configuration();  // Loads from "AtomNX/atom_gui.ini" and "AtomNX/sys_ipl.ini" if exists
	...
	FIL fp;																// NYX DEFAULT ICONS MAY CAUSE CONFLICTS    *****************************************************
	if (!f_open(&fp, "AtomNX/sys/res.pak", FA_READ))
	{
	...
	if (f_stat("AtomNX/res/icon_switch_custom.bmp", NULL))
		icon_switch = bmp_to_lvimg_obj("AtomNX/res/icon_switch.bmp");
	else
		icon_switch = bmp_to_lvimg_obj("AtomNX/res/icon_switch_custom.bmp");

	// If no custom payload icon exists, load normal.
	if (f_stat("AtomNX/res/icon_payload_custom.bmp", NULL))
		icon_payload = bmp_to_lvimg_obj("AtomNX/res/icon_payload.bmp");
	else
		icon_payload = bmp_to_lvimg_obj("AtomNX/res/icon_payload_custom.bmp");

	// Load background resource if any.
	hekate_bg = bmp_to_lvimg_obj("AtomNX/background.bmp");
