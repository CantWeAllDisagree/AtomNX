...
// MBOX Style Dump fully adapted and texts
static lv_res_t _create_window_dump_done(int error, char* dump_filenames)
{
	...
	lv_mbox_set_style(mbox, LV_MBOX_STYLE_BG, &mbox_bg);// MBOX Run Background Style
...
// MBOX SD Benchmark  (AtomNX Version) 
static lv_res_t _create_mbox_benchmark(bool sd_bench)
{
	...
	// MBOX Background style definition
	static lv_style_t mbox_bg;
	lv_style_copy(&mbox_bg, &lv_style_plain);
	mbox_bg.body.main_color = LV_COLOR_BLACK;
	mbox_bg.body.grad_color = mbox_darken.body.main_color;
	mbox_bg.body.opa = LV_OPA_40;
...
out:
	...
	// Buttonmap Style MBOX - Important here otherwise error!!!!!!!!!!!!!!!!!!!!
	lv_mbox_set_style(mbox, LV_MBOX_STYLE_BTN_BG, &lv_style_transp);// MBOX Buttons style Background
	lv_mbox_set_style(mbox, LV_MBOX_STYLE_BTN_REL, &btn_transp_rel);// MBOX Buttons style Release
	lv_mbox_set_style(mbox, LV_MBOX_STYLE_BTN_PR, &btn_transp_pr);// MBOX Buttons style Pressed
...
{
	lv_obj_t *win = gui_create_standard_window(SYMBOL_CHIP" Internal eMMC Info");
...
{
	lv_obj_t *win = gui_create_standard_window(SYMBOL_SD" microSD Card Info");
...
{
	lv_obj_t *win = gui_create_standard_window(SYMBOL_BATTERY_FULL" Battery Info");
...
static bool _lockpick_exists_check()
{
	...
	{
		FIL fp;
		if (f_open(&fp, "AtomNX/payloads/Lockpick_RCM.bin", FA_READ))
			goto out;
...
// HW info Page in window
lv_res_t create_win_info(lv_obj_t *btn)
{
	// Create a window to hold all the objects, with lv_layer_top() the window is not full image size, with lv_scr_act() ok
	lv_obj_t *win = lv_win_create(lv_scr_act(), NULL);
	lv_win_set_title(win, "Soc & HW Info");
	lv_page_set_scrl_layout(lv_win_get_content(win), LV_LAYOUT_PRETTY);//LV_LAYOUT_OFF
	lv_win_set_style(win, LV_WIN_STYLE_HEADER, &header_style);// Run Header Style
	lv_win_set_style(win, LV_WIN_STYLE_BG, &win_bg_style);// Run Background window

	// Own definition to close the first window, no more errors with x after autoclose win
	close_firstwin = lv_win_add_btn(win, NULL, SYMBOL_CLOSE, lv_win_close_action_firstwin);
	lv_obj_set_style(close_firstwin, LV_LABEL_STYLE_MAIN);// Style color close button


	lv_win_set_btn_size(win, 45);// Define size of header buttons

	// Info Page copied and adapted
	static lv_style_t h_style;
	lv_style_copy(&h_style, &lv_style_transp);
	h_style.body.padding.inner = 0;
	h_style.body.padding.hor = LV_DPI - (LV_DPI / 4);
	...
	lv_label_set_static_text(label_txt, "SoC & HW Infos");
	...
	btn = lv_btn_create(h1, NULL);//	lv_obj_t* btn = lv_btn_create(h1, NULL);
	if (hekate_bg)
	{
	...
	// Create Lockpick RCM button.
	lv_obj_t* btn2 = lv_btn_create(h1, btn);
	label_btn = lv_label_create(btn2, NULL);
	lv_label_set_static_text(label_btn, SYMBOL_KEY"  Lockpick RCM");
