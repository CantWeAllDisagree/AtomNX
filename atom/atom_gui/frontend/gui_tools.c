...
static lv_res_t _create_mbox_ums(usb_ctxt_t *usbs)
{
	...
	lv_obj_t *dark_bg = lv_obj_create(lv_scr_act(), NULL);// Background behind MBOX
	lv_obj_set_style(dark_bg, &mbox_darken);
	lv_obj_set_size(dark_bg, LV_HOR_RES, LV_VER_RES);
	...
	if (usbs->type == MMC_SD)
	{
		...
		{
		case 0:
			s_printf(txt_buf + strlen(txt_buf), "#FFFFFF SD Card#");
			break;
		case EMMC_GPP + 1:
			s_printf(txt_buf + strlen(txt_buf), "#FFFFFF emuMMC GPP#");
			break;
		case EMMC_BOOT0 + 1:
			s_printf(txt_buf + strlen(txt_buf), "#FFFFFF emuMMC BOOT0#");
			break;
		case EMMC_BOOT1 + 1:
			s_printf(txt_buf + strlen(txt_buf), "#FFFFFF emuMMC BOOT1#");
			break;
		}
	}
	else
	{
		...
		{
		case EMMC_GPP + 1:
			s_printf(txt_buf + strlen(txt_buf), "#FFFFFF eMMC GPP#");
			break;
		case EMMC_BOOT0 + 1:
			s_printf(txt_buf + strlen(txt_buf), "#FFFFFF eMMC BOOT0#");
			break;
		case EMMC_BOOT1 + 1:
			s_printf(txt_buf + strlen(txt_buf), "#FFFFFF eMMC BOOT1#");
			break;
		}
	}
